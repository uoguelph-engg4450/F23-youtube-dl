...
def add_link(url, add_link_entry_x, add_link_entry_y):
    pyautogui.click(x=add_link_entry_x, y=add_link_entry_y)  # Click on the link entry field
...
def remove_link(link_index, listbox_x, listbox_y):
    pyautogui.click(x=listbox_x, y=listbox_y + 20 * link_index)  # Click on the link at the given index
    pyautogui.press('tab', presses=3, interval=0.25)  # Navigate to the "Remove Selected Link" button
...
@pytest.mark.parametrize("output_path, output_name, links, path_exists, expected_result", [
...
def test_start_download(output_path, output_name, links, path_exists, expected_result):
    with patch('gui.path_entry.get', return_value=output_path), \
         patch('gui.name_entry.get', return_value=output_name), \
         patch('gui.listbox.get', return_value=links), \
         patch('gui.os.path.exists', return_value=path_exists), \
         patch('gui.threading.Thread') as mock_thread, \
         patch('gui.listbox.itemconfig') as mock_itemconfig, \
         patch('gui.path_entry.config') as mock_path_config, \
         patch('gui.name_entry.config') as mock_name_config, \
         patch('gui.status_label.config') as mock_status_label_config:
